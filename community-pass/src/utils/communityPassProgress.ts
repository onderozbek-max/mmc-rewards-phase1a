...
export interface CommunityPassProgress {
  ...
  /**
   * Phase 1A operationalizes exactly one benefit: the 250-point milestone.
   * These fields are the single source of truth for "is the member's real,
   * running benefit unlocked" — they intentionally ignore 1,000/3,000, which
   * are future roadmap milestones with no built benefit yet (see
   * `Milestone.operational` in communityPassData.ts).
   */
